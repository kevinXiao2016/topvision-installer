; *** Inno Setup version 5.1.0+ Chinese messages ***
;
...
WizardPassword=CDKEY
PasswordLabel1=本次安装需要输入CDKEY
PasswordLabel3=请输入CDKEY并单击『下一步』继续。CDKEY为大小写敏感字符。
PasswordEditLabel=CDKEY(&P)：
IncorrectPassword=您输入的CDKEY不正确, 请重新输入。
...
CloseApplications=自动关闭(&A)
DontCloseApplications=不关闭(&D)
ErrorCloseApplications=安装程序无法自动关闭所有应用程序。建议您在安装程序继续之前关闭所有应用程序。
...
AddonHostProgramNotFound=在您选的文件夹中无法找到%1.%n%n是否继续?
